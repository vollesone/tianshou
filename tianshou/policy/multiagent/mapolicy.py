    ...
    def process_fn(
        ...
        for agent, policy in self.policies.items():
            ...
            if not hasattr(tmp_batch.obs, "mask"):
                ...
                batch_obs_copy = np.copy(tmp_batch.obs)
                obs_next = np.roll(batch_obs_copy, -1, axis=0)    # left shift
                obs_next[-1] = obs_next[-2] # last element has no obs_next
                # so make it a copy of the previous element
